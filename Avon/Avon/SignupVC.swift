...
class SignupVC: ViewController {
	...
	func prepareUI() {
		...
		txtFN = UITextField(frame: CGRectMake(Constants.Size.SCREEN_WIDTH * 0.10,
			imgvSteps.frame.origin.y + imgvSteps.frame.size.height + 20,
			Constants.Size.SCREEN_WIDTH * 0.80,
			40))
		txtFN.borderStyle = .RoundedRect
		txtFN.placeholder = "First Name"
		scrollView.addSubview(txtFN)
		
		
		txtLN = UITextField(frame: CGRectMake(Constants.Size.SCREEN_WIDTH * 0.10,
			txtFN.frame.origin.y + txtFN.frame.size.height + 10,
			Constants.Size.SCREEN_WIDTH * 0.80,
			40))
		txtLN.borderStyle = .RoundedRect
		txtLN.placeholder = "Last Name"
		scrollView.addSubview(txtLN)
		
		txtEM = UITextField(frame: CGRectMake(Constants.Size.SCREEN_WIDTH * 0.10,
			txtLN.frame.origin.y + txtLN.frame.size.height + 10,
			Constants.Size.SCREEN_WIDTH * 0.80,
			40))
	...
	// MARK: - SIGNIN
	func attemptSignup() {
		if txtFN.text?.isEmpty == true {
			UIAlertController.showMessage(self, title: "Sign Up Failed", message: "Please enter your first name")
			return
		}

		if txtLN.text?.isEmpty == true {
			UIAlertController.showMessage(self, title: "Sign Up Failed", message: "Please enter your last name")
			return
		}

		if txtEM.text?.isEmpty == true {
			UIAlertController.showMessage(self, title: "Sign Up Failed", message: "Please enter your email address")
			return
		}
	...
	func signupUser() {
		...
		let strURL = "https://avonmillionsteps.herokuapp.com/api/users/register"
